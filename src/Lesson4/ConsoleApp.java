...
public class ConsoleApp {
    ...
    public static int roomSizeMin = 4;
    ...
    public static int enemyValueMinAttack = 1;
    public static int enemyValueMaxAttack = 5;
    ...
    public static void spawnEnemies(){
        ...
        enemyAttack = randomValue(enemyValueMinAttack, enemyValueMaxAttack);
    ...
    public static void playerNextMoveAction(int lastPosX, int lastPosY, int nextPosX, int nextPosY) {   //Метод доработан. Бой проходит автоматически, но описывается каждый его шаг.
        if (invisibleRoom[nextPosY][nextPosX] == enemy) {
            System.out.println("ALLERT! You are facing an enemy! The battle has begun!!!");
                while (enemyHP > 0 && playerHP > 0) {
                    if (enemyHP >= playerAttack) {
                        enemyHP -= playerAttack;
                    } else enemyHP = 0;
                    System.out.println("You have dealt " + playerAttack + " damage to the enemy. Enemies HP is " + enemyHP + " now.");
                    if (enemyHP > 0) {
                        playerHP -= enemyAttack;
                        System.out.println("Enemy give damage " + enemyAttack + ". Player HP now is " + playerHP);
                    }

                }
            System.out.println("Congratulations! You`ve defeated the enemy!!!");
    ...
    public static boolean checkPlayerMove(int currentX, int currentY, int nextX, int nextY) { // Метод доработан. Теперь нельзя ходить на клетку, на которой игрок уже был. Вроде на уроке хотели так сделать )))
        if (nextX >= 0 && nextX < roomWidth && nextY >= 0 && nextY < roomHeight && room[nextX][nextY] != readyCell) {
